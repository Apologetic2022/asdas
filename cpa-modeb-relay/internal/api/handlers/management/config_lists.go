...
func (h *Handler) PatchCursorKey(c *gin.Context) {
	...
	var body struct {
		Index *int            `json:"index"`
		Match *string         `json:"match"`
		Value *cursorKeyPatch `json:"value"`
	}
	if err := c.ShouldBindJSON(&body); err != nil || body.Value == nil {
		c.JSON(400, gin.H{"error": "invalid body"})
		return
	...
	defer h.mu.Unlock()
	targetIndex := -1
	if body.Index != nil && *body.Index >= 0 && *body.Index < len(h.cfg.CursorKey) {
		targetIndex = *body.Index
	}
	if targetIndex == -1 && body.Match != nil {
		match := strings.TrimSpace(*body.Match)
		for i := range h.cfg.CursorKey {
			if h.cfg.CursorKey[i].APIKey == match {
				targetIndex = i
				break
			}
		}
	...
	entry := h.cfg.CursorKey[targetIndex]
	if body.Value.APIKey != nil {
		entry.APIKey = strings.TrimSpace(*body.Value.APIKey)
	}
...
func (h *Handler) DeleteCursorKey(c *gin.Context) {
	...
	if val := strings.TrimSpace(c.Query("api-key")); val != "" {
		if baseRaw, okBase := c.GetQuery("base-url"); okBase {
			base := strings.TrimSpace(baseRaw)
			out := make([]config.CursorKey, 0, len(h.cfg.CursorKey))
			for _, v := range h.cfg.CursorKey {
				if strings.TrimSpace(v.APIKey) == val && strings.TrimSpace(v.BaseURL) == base {
					continue
				}
				out = append(out, v)
		...
		matchCount := 0
		for i := range h.cfg.CursorKey {
			if strings.TrimSpace(h.cfg.CursorKey[i].APIKey) == val {
				matchCount++
				if matchIndex == -1 {
					matchIndex = i
				}
		...
		h.cfg.CursorKey = append(h.cfg.CursorKey[:matchIndex], h.cfg.CursorKey[matchIndex+1:]...)
		h.cfg.SanitizeCursorKeys()
		h.persistLocked(c)
		return
	}
	if idxStr := c.Query("index"); idxStr != "" {
		var idx int
		_, err := fmt.Sscanf(idxStr, "%d", &idx)
		if err == nil && idx >= 0 && idx < len(h.cfg.CursorKey) {
			h.cfg.CursorKey = append(h.cfg.CursorKey[:idx], h.cfg.CursorKey[idx+1:]...)
			h.cfg.SanitizeCursorKeys()
			h.persistLocked(c)
			return
		}
...
func normalizeCursorKey(entry *config.CursorKey) {
	if entry == nil {
		return
	}
	entry.APIKey = strings.TrimSpace(entry.APIKey)
